...
const (
	...
	// Model samples traces based on a simple model/score.
	Model PolicyType = "model"
)

// ModelCfg holds configuration for model-based sampling.
//
// Current supported model type:
//   - linear: score = intercept + sum(weights[feature] * feature_value)
//     sampled if score >= threshold.
//
// Supported feature names:
// - duration_ms: end-to-start duration across all spans in trace
// - span_count: number of spans seen for the trace
// - has_error: 1 if any span has StatusCode=ERROR or http.status_code >= 500, else 0
...
type BasePolicy struct {
	...
	// Model-based sampling configuration. Only used when Type == "model".
	ModelCfg *ModelCfg `mapstructure:"model"`
...
type Config struct {
	...
	// NumTraces is the number SubPoliciesof traces kept on memory. Typically most of the data
...
func (c *Config) Validate() error {
	...
	for i := range c.PolicyCfgs {
		pg := &c.PolicyCfgs[i]

		if err := validateAndNormalizeBasePolicy(&pg.BasePolicy); err != nil {
			errs = append(errs, fmt.Errorf("%s: %w", pg.Name, err))
		}

		for j := range pg.SubPolicies {
			sp := &pg.SubPolicies[j]
			if err := validateAndNormalizeBasePolicy(sp); err != nil {
				errs = append(errs, fmt.Errorf("%s/%s: %w", pg.Name, sp.Name, err))
			}
		}
	}
...
func validateAndNormalizeBasePolicy(p *BasePolicy) error {
	var errs []error

	if p.Priority == 0 {
		// priority must be explicitly assigned
		errs = append(errs, fmt.Errorf("priority must be greater than 0"))
	}

	if p.Type != Model {
		// Normalize: ignore model config if policy isn't model.
		p.ModelCfg = nil
		if len(errs) != 0 {
			return multierr.Combine(errs...)
		}
	...
	// Type == Model
	if p.ModelCfg == nil {
		errs = append(errs, fmt.Errorf("model config is required when type=model"))
		if len(errs) != 0 {
			return multierr.Combine(errs...)
		}
		return nil
	...
	// Allow empty model type to default to linear to avoid unnecessary breakage.
	if p.ModelCfg.Type != "" && p.ModelCfg.Type != "linear" {
		errs = append(errs, fmt.Errorf("unsupported model.type=%q (supported: linear)", p.ModelCfg.Type))
	}

	if math.IsNaN(p.ModelCfg.Threshold) || math.IsInf(p.ModelCfg.Threshold, 0) {
		errs = append(errs, fmt.Errorf("model.threshold must be a finite number"))
	}
	...
	if len(p.ModelCfg.Weights) == 0 {
		errs = append(errs, fmt.Errorf("model.weights must not be empty"))
	} else {
		...
		for feature, weight := range p.ModelCfg.Weights {
			if _, ok := allowed[feature]; !ok {
				errs = append(errs, fmt.Errorf("model.weights contains unsupported feature %q", feature))
				continue
			}
			if math.IsNaN(weight) || math.IsInf(weight, 0) {
				errs = append(errs, fmt.Errorf("model.weights[%q] must be a finite number", feature))
			}
		}
	}
