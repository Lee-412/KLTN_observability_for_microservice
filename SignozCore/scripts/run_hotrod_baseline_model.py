...
Optional:
    python3 scripts/run_hotrod_baseline_model.py --hotrod-seconds 90
"""
...
def _docker_compose_cmd() -> List[str]:
    # This repo currently uses docker-compose (v1) in the Linux lab environment.
    return ["docker-compose"]


def _utc_now_rfc3339() -> str:
    # docker logs --since supports RFC3339 timestamps.
...
def _write_proof_report(
    *,
    ...
    model_snapshot: Path,
    model_logs_since: str,
) -> Path:
    out_dir = root / "reports" / "sampler-proof"
    out_dir.mkdir(parents=True, exist_ok=True)
    ts = datetime.now().strftime("%Y%m%d-%H%M%S")
    out_path = out_dir / f"{ts}-sampler-proof.txt"

    def size_or_na(p: Path) -> str:
        try:
            return str(p.stat().st_size)
        except FileNotFoundError:
            return "NA"

    # Keep the grep tight: we want undeniable signals.
    proof_cmd = (
        "docker logs signoz-otel-collector --since "
        + model_logs_since
        + " 2>&1 | grep -E \"model tail-sampling policy using|adaptive model sampler (initialized|first recompute)|Sampling policy evaluation completed\" | tail -n 200 || true"
    )
    proc = subprocess.run(proof_cmd, shell=True, capture_output=True, text=True)

    lines: List[str] = []
    lines.append(f"baseline_snapshot: {baseline_snapshot}")
    lines.append(f"baseline_bytes: {size_or_na(baseline_snapshot)}")
    lines.append(f"model_snapshot: {model_snapshot}")
    lines.append(f"model_bytes: {size_or_na(model_snapshot)}")
    lines.append("")
    lines.append(f"collector_logs_since: {model_logs_since}")
    lines.append(f"collector_proof_cmd: {proof_cmd}")
    lines.append("")
    lines.append("---- collector proof (grep) ----")
    lines.append((proc.stdout or "").rstrip("\n"))
    if proc.stderr:
        lines.append("")
        lines.append("---- collector proof stderr ----")
...
def main() -> int:
    ...
    ap.add_argument("--skip-compare", action="store_true", help="Skip scripts/run_compare_and_save.py")
    args = ap.parse_args()
    ...
    try:
        ...
        proc = _run(compose + ["up", "-d", "--force-recreate", "otel-collector"], cwd=compose_dir)
        ...
        # Capture proof logs for the model pass.
        model_logs_since = _utc_now_rfc3339()
        proc = _run(compose + ["up", "-d", "--force-recreate", "otel-collector"], cwd=compose_dir)
        ...
        proof_path = _write_proof_report(
            ...
            model_logs_since=model_logs_since,
        )
        print(f"Sampler proof report: {proof_path}")
