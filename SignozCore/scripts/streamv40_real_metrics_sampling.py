...
def build_real_metrics_stream(metric_dir: str) -> Dict[int, MetricsSnapshot]:
    """Parse real paper metrics into second->service->normalized metric snapshots."""
    if not metric_dir:
        return {}
    ...
    for service, rows in rows_by_service.items():
        ...
        per_sec_acc: Dict[int, Dict[str, List[float]]] = defaultdict(lambda: {
            "cpu": [],
            "memory": [],
            "latency": [],
            "workload": [],
...
def _service_metric_anomaly(
    ...
    raw = 0.35 * z_lat + 0.25 * z_cpu + 0.20 * z_mem + 0.20 * z_work
    svc_score = _sigmoid(raw)
    if error_rate > 0.01:
        svc_score += 0.20
...
def _composite_v3_metrics_strictcap(
    ...
    for p in traces:
        ...
        # v4.0 fusion: real metrics distress, no change to downstream diversity/cap logic.
        p_s_v3 = p_s
        ...
        p_s = p_s_v3 + 0.35 * distress * (1.0 - p_s_v3)
        if distress > 0.75:
            p_s = max(p_s, 0.75)
...
def _composite_v40_real_metrics_sampling(
    points: List[TracePoint],
    budget_pct: float,
    preference_vector: PreferenceVector,
    scenario_windows: List[Tuple[str, int, int]],
