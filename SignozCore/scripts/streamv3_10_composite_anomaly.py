...
# ==========================================
# METHOD 3: v3.10 (BALANCED HYBRID DESIGN)
# Strict adherence to paper-level prompt
# ==========================================

# FIX [1]: Data Starvation Prevention
...
def update_service_state(
    state: ServiceAnomalyState, current_ts: float, latency: float, has_error: bool, force_evict: bool = False
) -> None:
...
def compute_anomaly_score(state: ServiceAnomalyState, latency: float) -> float:
    ...
    std_lat = min(math.sqrt(variance), STD_CLIP)
...
def _median(values: List[float]) -> float:
    ...
    xs = sorted(values)
    mid = len(xs) // 2
    if len(xs) % 2 == 1: return float(xs[mid])
...
def _rank_project(values: List[float]) -> float:
    ...
    weights = [0.5, 0.3, 0.2]
    projected = 0.0
    for weight, value in zip(weights, ranked): projected += weight * value
    return projected

def _compute_metric_signal_details(trace: TracePoint, preference_vector: PreferenceVector, metric_stats: MetricStats) -> Tuple[float, float, float]:
    if not trace.services: return 0.0, 0.0, 0.0
    effective_services = [s for s in trace.services if s in preference_vector and int(metric_stats.get(s, {}).get("history_count", 0)) > 10]
    base_coverage = len(effective_services) / float(len(trace.services)) if trace.services else 0.0
    if not effective_services: return 0.0, base_coverage, 0.0
    service_scores, history_counts = [], []
    for service in effective_services:
        metric_vector = preference_vector.get(service, {})
        raw_z = [float(metric_vector.get("latency", 0.0)), float(metric_vector.get("error", 0.0)), float(metric_vector.get("throughput", 0.0)), float(metric_vector.get("burst", 0.0))]
...
def compute_metric_score(trace: TracePoint, preference_vector: PreferenceVector, metric_stats: MetricStats) -> Tuple[float, float]:
    metric_score, coverage_ratio, _ = _compute_metric_signal_details(trace, preference_vector, metric_stats)
...
def _trace_preference_score(p: TracePoint, preference_vector: PreferenceVector) -> float:
    if not p.services or not preference_vector: return 0.0
    best = 0.0
    for svc in p.services:
        metric_vector = preference_vector.get(svc, {})
        service_score = 0.4 * float(metric_vector.get("latency", 0.0)) + 0.3 * float(metric_vector.get("error", 0.0)) + 0.2 * float(metric_vector.get("throughput", 0.0)) + 0.1 * float(metric_vector.get("burst", 0.0))
        best = max(best, service_score)
    return best

def _composite_v3_metrics_strictcap(
    points: List[TracePoint], budget_pct: float, preference_vector: PreferenceVector,
    scenario_windows: List[Tuple[str, int, int]], incident_anchor_sec: Optional[int],
    seed: int, metric_stats: Optional[Dict[str, Tuple[float, float]]] = None,
    incident_services: Optional[Set[str]] = None, min_incident_traces_per_scenario: int = 1,
    online_soft_cap: bool = False, lookback_sec: int = 60, alpha: float = 1.2,
    gamma: float = 0.8, anomaly_weight: float = 0.7, ablation_mode: str = "full",
) -> Set[str]:
    total = len(points)
    if total == 0: return set()
    beta = _clamp(budget_pct / 100.0, 0.0, 1.0)
    if beta <= 0.0: return set()
    ...
    dur_lo, dur_hi = min(p.duration_ms for p in traces), max(p.duration_ms for p in traces)
    span_lo, span_hi = min(p.span_count for p in traces), max(p.span_count for p in traces)

    def _norm(v: float, lo: float, hi: float) -> float:
        if hi <= lo: return 0.0
        return _clamp((v - lo) / (hi - lo), 0.0, 1.0)
    def _dur_bucket(d_ms: float) -> str:
        if d_ms < 100: return "lt100"
        if d_ms < 500: return "100_500"
        if d_ms < 2000: return "500_2000"
        return "ge2000"
    ...
    metric_state: Dict[str, Dict[str, Any]] = defaultdict(
        lambda: {"latency": deque(maxlen=history_size), "error": deque(maxlen=history_size), "throughput": deque(maxlen=history_size), "burst": deque(maxlen=history_size), "ema": 0.0, "sec": None, "count": 0.0}
    )
    anomaly_state: Dict[str, ServiceAnomalyState] = defaultdict(
        lambda: ServiceAnomalyState(latencies=deque(), errors=deque(), ema_latency=None, last_eviction_ts=0.0, sum_latency=0.0, sum_latency_sq=0.0, sum_errors=0.0)
    )
    ...
    for p in traces:
        ...
        while seen_win and seen_win[0][0] < sec - lookback_sec + 1: seen_win.popleft()
        while kept_win and kept_win[0][0] < sec - lookback_sec + 1: kept_win.popleft()
        ...
        time_prox = 0.5 if incident_anchor_sec is None else math.exp(-abs(sec - incident_anchor_sec) / 60.0)
        ...
        for service in sorted(p.services):
            ...
            current_count = 0.0 if state["sec"] != sec else float(state["count"])
            throughput = math.log1p(current_count + 1.0)
            ema = 0.2 * throughput + 0.8 * float(state["ema"])
            burst = throughput - ema
            
            latency_hist, error_hist = list(state["latency"]), list(state["error"])
            throughput_hist, burst_hist = list(state["throughput"]), list(state["burst"])
            
            latency_med, latency_mad = _median(latency_hist), _mad(latency_hist)
            error_med, error_mad = _median(error_hist), _mad(error_hist)
            throughput_med, throughput_mad = _median(throughput_hist), _mad(throughput_hist)
            burst_med, burst_mad = _median(burst_hist), _mad(burst_hist)
            ...
            historical_metric_stats[service] = {
                "latency": (latency_med, latency_mad), "error": (error_med, error_mad), "throughput": (throughput_med, throughput_mad), "burst": (burst_med, burst_mad), "history_count": len(latency_hist),
            }

        hist_err, hist_lat, hist_inc = [x[1] for x in seen_win], [x[2] for x in seen_win], [x[3] for x in seen_win]
        ...
        sg_err, sg_lat, sg_inc = _robust_sigma(hist_err), _robust_sigma(hist_lat), _robust_sigma(hist_inc)
        ...
        # Baseline p_s calculation
        p_s = compute_sampling_probability(p, {
            "incident_service": incident_service, "lat_norm": lat_pressure, "err_flag": err_flag, "time_prox": time_prox,
            "alpha_eff": alpha_eff, "z_mix": z_mix, "p_min": 0.02, "p_max": 0.98,
            "preference_vector": historical_preference_vector, "metric_stats": historical_metric_stats,
        })

        # --- v3.10 ANOMALY SIGNAL ---
        ...
        if p.services:
            ...
            for service in sorted(p.services):
                ...
                update_service_state(service_state, float(sec), float(p.duration_ms), bool(p.has_error), force_evict=(global_trace_counter % EVICTION_COUNTER_MOD == 0))
                anomaly_scores.append(compute_anomaly_score(service_state, float(p.duration_ms)))
            if anomaly_scores:
                norm_anomaly = max(anomaly_scores)
                
        # FIX [2]: REMOVE DILUTION -> MULTIPLICATIVE BOOST
        p_s_before = p_s
        p_s = _clamp(p_s * (1.0 + norm_anomaly * ANOMALY_WEIGHT), 0.02, 0.98)

        # Incident Floor
        incident_floor = 0.0
        if incident_service > 0.0:
            if (err_flag > 0.0) and (time_prox >= 0.55): incident_floor = 0.62
            elif (time_prox >= 0.78) and (lat_pressure >= 0.58): incident_floor = 0.56
            elif (time_prox >= 0.88) and (lat_pressure >= 0.50): incident_floor = 0.50
        p_s = max(p_s, incident_floor)

        # Clustering & Mass Calculation
        ...
        if best_key is not None and best_sim >= cluster_merge_threshold:
            ...
            effective_mass = cluster_counts.get(assigned_key, 0) * best_sim
        else:
            assigned_key = tuple(sorted(toks))
            effective_mass = 0.0

        # FIX [3]: UNLOCK GAMMA PATH (CRITICAL)
        # Step 1: Base Config
        if incident_service > 0.0:
            base_gamma = max(0.35, gamma * 0.55)
            base_floor = 0.20
        else:
            base_gamma = gamma
            base_floor = 0.05
            
        # Step 2: Apply Anomaly ALWAYS
        gamma_reduction = 1.0 - (0.6 * norm_anomaly)
        gamma_eff = base_gamma * gamma_reduction
        pd_floor = min(0.25, base_floor + (0.3 * norm_anomaly))

        p_d = _clamp((1.0 + effective_mass) ** (-gamma_eff), pd_floor, 1.0)
        
        # FIX [5]: DEBUG LOGGING
        if global_trace_counter % 5000 == 0:
            print(f"[DEBUG v3.10] anomaly={norm_anomaly:.3f} | gamma_eff={gamma_eff:.3f}/{base_gamma:.3f} | p_s={p_s_before:.3f}->{p_s:.3f}")

        # Budget & Gating
        theta = (len(kept_ids) / seen) if seen > 0 else 0.0
        if beta <= 0.0015: ps_or_floor = 0.12
        elif beta <= 0.01: ps_or_floor = 0.20
        else: ps_or_floor = 0.30
        ...
        trace_metric_score, _, _ = _compute_metric_signal_details(p, historical_preference_vector, historical_metric_stats)
    ...
    if kept_ids:
        ...
        if len(kept_err) < needed_err:
            ...
            if add_ids:
                ...
                for tid in cand_drop[:drop_n]: kept_ids.discard(tid)
                for tid in add_ids[:drop_n]: kept_ids.add(tid)

    if not scenario_windows or min_incident_traces_per_scenario <= 0:
        ranked_ids = sorted(kept_ids, key=lambda tid: (-(1.0 if by_id.get(tid) and by_id[tid].has_error else 0.0), -p_s_final_by_id.get(tid, 0.0), tid))
    ...
    for idx, (_sid, st, ed) in enumerate(scenario_windows):
        ...
        if ids:
            ...
            for tid in ids: scenario_membership[tid].add(idx)
    ...
    if target_count < non_empty_scenarios * floor_each: floor_each = 0
    def _priority(tid: str) -> Tuple[float, float, float, int, str]:
        p = by_id.get(tid)
        if p is None: return (0.0, 0.0, 0.0, 0, tid)
        return (1.0 if p.has_error else 0.0, float(p.duration_ms), float(p.span_count), len(scenario_membership.get(tid, set())), tid)
    ...
    if floor_each > 0:
        for idx, ids in enumerate(scenario_ids_by_idx):
            ...
            if len(present) >= floor_each: continue
            ...
            if not candidates:
                scenario_floor_fail_count += 1
                continue
            for tid in candidates[:need]: final_ids.add(tid)
            if len([tid for tid in final_ids if tid in ids]) < floor_each:
                scenario_floor_fail_count += 1
                
    if len(final_ids) <= target_count: return final_ids
    scenario_counts = [0] * len(scenario_ids_by_idx)
    for tid in final_ids:
        for idx in scenario_membership.get(tid, set()): scenario_counts[idx] += 1
    def _can_remove(tid: str) -> bool:
        if floor_each <= 0: return True
        for idx in scenario_membership.get(tid, set()):
            if scenario_counts[idx] <= floor_each: return False
        return True
        
    removable = sorted(final_ids, key=_priority)
    for tid in removable:
        if len(final_ids) <= target_count: break
        if not _can_remove(tid): continue
        final_ids.remove(tid)
        for idx in scenario_membership.get(tid, set()): scenario_counts[idx] -= 1
        
    if len(final_ids) > target_count:
        removable_ids = sorted(final_ids, key=lambda tid: (-(1.0 if by_id.get(tid) and by_id[tid].has_error else 0.0), -p_s_final_by_id.get(tid, 0.0), tid))
