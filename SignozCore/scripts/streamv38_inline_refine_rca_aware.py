"""
Stream v3.7 — RCA-aware sampling (Direction A).

Core idea: v3.5 treats all error traces equally in post-hoc ranking.
MicroRank needs error traces through root-cause (gold) more than
error traces through non-root-cause (noise).

Four-quadrant RCA contrast model:
  - Gold:   error + touches suspicious service  → high value for ef(root_cause)
  - Silver: success + no suspicious service     → good contrast for ep(non-root)
  - Noise:  error + no suspicious service       → hurts spectrum (raises ef for non-root)
  - Mixed:  success + touches suspicious        → moderate (calibrates ep for root)

Changes from v3.5:
  1. Dynamic suspicious-service tracking (incident_services + rolling error rate)
  2. Per-trace rca_value from 4-quadrant model
  3. rca_boost applied to p_s during streaming
  4. score_by_id weighted by rca_value
  5. Post-hoc ranking prioritises rca_value over raw error flag
  6. Error-density swap prefers RCA-relevant error traces
"""
...
def _rca_quadrant_value(
    ...
    """Assign RCA-contrast value based on error × suspicious quadrant."""
    if has_error and suspicious_hit > 0:
        return 0.85 + 0.15 * suspicious_ratio          # Gold
    elif (not has_error) and suspicious_hit == 0:
        return 0.55                                      # Silver — clean contrast
    elif has_error and suspicious_hit == 0:
        return 0.10                                      # Noise — hurts spectrum
    else:
        return 0.35                                      # Mixed — success through suspicious
...
def _composite_v37_rca_aware(
    ...
    for p in traces:
        ...
        # --- v3.7: dynamic suspicious set from incident_services + high-error services ---
        dynamic_suspicious = set(incident_services)
        for svc in p.services:
            if svc_trace_count[svc] > 10:
        ...
        # --- v3.5 metric state computation (unchanged) ---
        ...
        # --- v3.7: RCA-aware boost to sampling probability ---
        ...
        # --- v3.7: update RCA tracking counters (after read, before next trace) ---
        for svc in p.services:
    ...
    # --- v3.7 modified: error density protection prefers RCA-relevant errors ---
    if kept_ids:
        ...
        if len(kept_err) < needed_err:
            ...
            # v3.7: prefer error traces with high rca_value (gold > noise)
    ...
    # --- v3.7 modified: post-hoc ranking by rca_value first ---
...
def _composite_v38_inline_refine_rca_aware(
    points: List[TracePoint],
    budget_pct: float,
    preference_vector: PreferenceVector,
    scenario_windows: List[Tuple[str, int, int]],
    ...
    """v3.8 revised: v3.7 backbone + service-quota floor on p_s.

    Key difference from v3.7: when a suspicious service has fewer than
    ``quota_k`` traces in the kept set, new traces touching that service
    get a p_s floor boost.  This prevents sparse-service starvation that
    degrades MicroRank spectrum analysis (especially on train-ticket).

    No buffer/trim — direct accept/reject preserves v3.5 diversity.
    """
    del anomaly_weight
    ...
    # --- v3.8 addition: service-quota tracking ---
    quota_k = 2 if target_count >= 30 else 1
    suspicious_services_global: Set[str] = set(incident_services)
    service_kept_count: Dict[str, int] = defaultdict(int)
    ...
    for p in traces:
        ...
        for svc in p.services:
            if svc_trace_count[svc] > 10:
                ...
                if err_rate > high_error_service_threshold:
                    dynamic_suspicious.add(svc)
                    suspicious_services_global.add(svc)
        ...
        # --- v3.8: service-quota floor ---
        # If any suspicious service this trace touches is under-represented
        # in kept set, boost p_s to ensure MicroRank gets enough coverage.
        for svc in p.services:
            if svc in suspicious_services_global and service_kept_count.get(svc, 0) < quota_k:
                p_s = max(p_s, 0.50)
                break
        ...
        if keep:
            kept_ids.add(p.trace_id)
            kept_win.append((sec, p.has_error))
            # --- v3.8: update service quota counters ---
            for svc in p.services:
                if svc in suspicious_services_global:
                    service_kept_count[svc] += 1
        p_s_final_by_id[p.trace_id] = p_s
