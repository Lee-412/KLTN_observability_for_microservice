...
def _composite_v352_service_coverage(
    points: List[TracePoint],
    budget_pct: float,
    preference_vector: PreferenceVector,
    ...
    # v3.52: Keep minimum error density (from v3.5 baseline)
    if kept_ids:
    ...
    # v3.52: Service-coverage maximization post-hoc swap
    # Maximize distinct service representation to improve MicroRank's PageRank graph
    if kept_ids and len(kept_ids) >= 3:
        # Count service representation in current kept set
        ...
        # All services seen in entire dataset
        all_svcs = set()
        for tp in traces:
            all_svcs.update(tp.services)
        # Services missing from kept set
        missing_svcs = all_svcs - set(svc_count_kept.keys())
        # Try to swap in traces covering missing services
        max_swaps = max(1, len(kept_ids) // 5)  # swap up to 20% of kept set
        swaps_done = 0
        for _ in range(max_swaps):
            if not missing_svcs:
                break
            # Find best candidate covering missing services
            best_cand = None
            best_new_svcs = 0
            ...
            if best_cand is None or best_new_svcs == 0:
                break
            # Find worst trace to drop (most redundant services, lowest score)
            # Exclude error traces from drop candidates
            drop_cands = []
            for tid in kept_ids:
                tp = by_id.get(tid)
                if tp and not tp.has_error:
                    min_svc_count = min(svc_count_kept.get(s, 0) for s in tp.services) if tp.services else 0
                    redundancy = min_svc_count
                    drop_cands.append((tid, redundancy, score_by_id.get(tid, 0.0)))
            if not drop_cands:
                break
            # Drop the trace with highest redundancy (then lowest score)
