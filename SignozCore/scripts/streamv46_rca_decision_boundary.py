"""
Stream v4.6 -- RCA Decision-Boundary Redesign.

Backbone: v3.9 (v3.7 + tuned quadrant/threshold/swap).

v3.9 showed that signal tuning is correct in direction (2.5%% improved)
but 0.1%% hit a hard ceiling caused by the AND-gate decision boundary
(u < p_s AND u < p_d).  v4.6 attacks this bottleneck directly with
three complementary changes:

  Direction A -- OR-gated RCA keep (decision boundary):
    Gold error traces (has_error=True, rca_value >= threshold) bypass
    the AND gate; they are kept if EITHER p_s OR p_d passes.
    This prevents the diversity gate from blocking root-cause traces.

  Direction B -- Budget-adaptive aggressive mode:
    At ultra-low budgets (beta <= 0.15%%), shift from explore to exploit:
    higher rca_weight, lower normal_ratio_target, tighter cluster merge.

  Direction C -- Gold trace reservation (post-hoc):
    Reserve ~40%% of target_count for Gold error traces. If the
    streaming pass under-collected Gold traces, swap in the best
    Gold candidates and drop lowest-scored non-Gold traces.
"""
...
def _rca_quadrant_value(
    ...
    v3.9 tuning: Noise raised from 0.10  to  0.28 to avoid over-punishing
    early error traces whose root-cause service hasn't entered the
    suspicious set yet.
    """
    if has_error and suspicious_hit > 0:
        return 0.80 + 0.10 * suspicious_ratio          # Gold
    elif (not has_error) and suspicious_hit == 0:
        return 0.50                                      # Silver -- clean contrast
    elif has_error and suspicious_hit == 0:
        return 0.28                                      # Noise -- softer penalty
    else:
        return 0.40                                      # Mixed -- success through suspicious
...
def _composite_v46_rca_decision_boundary(
    points: List[TracePoint],
    budget_pct: float,
    preference_vector: PreferenceVector,
    ...
    # --- v4.6 Direction B: budget-adaptive aggressive mode ---
    if beta <= 0.0015:
        rca_weight = max(rca_weight, 0.65)
        normal_ratio_target = 0.15
    ...
    for p in traces:
        ...
        if theta > beta:
            if (not _DISABLE_OVERRIDE) and is_high_confidence_anomaly:
                ...
                # v4.6 Direction A: OR-gated RCA keep for gold error traces
                is_gold = p.has_error and rca_value >= gold_rca_threshold
                if is_gold:
                    keep = (u < p_s) or (u < p_d)
                else:
    ...
    # --- v4.6 Direction C: Gold trace reservation ---
    if kept_ids and target_count >= 4:
        gold_quota = max(2, int(target_count * 0.40))
        gold_in_kept = [
            tid for tid in kept_ids
            if by_id.get(tid) and by_id[tid].has_error
            and rca_value_by_id.get(tid, 0.0) >= gold_rca_threshold
        ]
        if len(gold_in_kept) < gold_quota:
            deficit = gold_quota - len(gold_in_kept)
            cand_gold = [
                tid for tid, tp in by_id.items()
                if tid not in kept_ids and tp.has_error
                and rca_value_by_id.get(tid, 0.0) >= gold_rca_threshold
            ]
            cand_gold.sort(
            ...
            if add_gold:
                ...
                droppable.sort(key=lambda tid: score_by_id.get(tid, 0.0))
                drop_n = min(len(add_gold), len(droppable))
                for tid in droppable[:drop_n]:
                    kept_ids.discard(tid)
                for tid in add_gold[:drop_n]:
                    kept_ids.add(tid)
