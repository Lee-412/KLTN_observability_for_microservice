...
def _composite_v351_contrast_rebalance(
    points: List[TracePoint],
    budget_pct: float,
    preference_vector: PreferenceVector,
    ...
    # v3.51: Contrast-aware post-hoc rebalancing
    # Keep minimum error density (from v3.5 baseline)
    if kept_ids:
    ...
    # v3.51: Enforce max error ratio cap — swap excess error traces for success traces
    if kept_ids:
        kept_err_ids = [tid for tid in kept_ids if by_id.get(tid) and by_id[tid].has_error]
        kept_ok_ids = [tid for tid in kept_ids if by_id.get(tid) and not by_id[tid].has_error]
        ...
        if current_err_ratio > max_error_ratio:
            ...
            if excess > 0:
                # Remove lowest-score error traces
                kept_err_ids.sort(key=lambda tid: score_by_id.get(tid, 0.0))
