...
def _composite_v391_contrast_rebalance(
    points: List[TracePoint],
    budget_pct: float,
    preference_vector: PreferenceVector,
    ...
    # v3.91: Contrast-aware post-hoc rebalancing -- enforce max error ratio
    if kept_ids:
        kept_err_ids = [tid for tid in kept_ids if by_id.get(tid) and by_id[tid].has_error]
        current_err_ratio = len(kept_err_ids) / len(kept_ids) if kept_ids else 0.0
        if current_err_ratio > max_error_ratio:
            max_err_count = max(1, int(round(len(kept_ids) * max_error_ratio)))
            excess = len(kept_err_ids) - max_err_count
            if excess > 0:
                # Remove lowest-rca-value error traces
                kept_err_ids.sort(key=lambda tid: (rca_value_by_id.get(tid, 0.0), score_by_id.get(tid, 0.0)))
                drop_err = kept_err_ids[:excess]
                # Find best success traces to swap in
                cand_ok = [tid for tid, tp in by_id.items() if tid not in kept_ids and not tp.has_error]
                cand_ok.sort(key=lambda tid: score_by_id.get(tid, 0.0), reverse=True)
                swap_in = cand_ok[:excess]
                swap_n = min(len(drop_err), len(swap_in))
                for tid in drop_err[:swap_n]:
                    kept_ids.discard(tid)
                for tid in swap_in[:swap_n]:
                    kept_ids.add(tid)
