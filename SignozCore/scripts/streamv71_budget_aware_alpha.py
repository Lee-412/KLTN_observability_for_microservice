"""
Stream v7.1 — Budget-Aware Alpha Sampling.

v3.9 backbone with two targeted budget-adaptive changes:

1. Budget-aware theta threshold (admission gate):
   Instead of the AND gate kicking in when theta > beta (100% of quota),
   it kicks in earlier at high budgets:
     0.1%  → theta_strict = beta         (100%: same as v3.9)
     1.0%  → theta_strict = 0.85 * beta  ( 85%: slightly earlier)
     2.5%  → theta_strict = 0.70 * beta  ( 70%: more aggressive)
   Effect: At 2.5% budget, we start applying the strict AND gate when we've
   admitted 70% of the quota, ensuring harder diversity checks earlier in the
   stream. This reduces noise admission in the long tail.

2. Budget-aware post-hoc score weights:
   At tight budget (0.1%): precision-first → trust system signal heavily.
   At large budget (2.5%): coverage-first → trust diversity + RCA more.
     0.1%:  w_sys=0.70, w_div=0.10, w_rca=0.20
     1.0%:  w_sys=0.55, w_div=0.15, w_rca=0.30
     2.5%:  w_sys=0.40, w_div=0.25, w_rca=0.35

All other logic is identical to v3.9 (quadrant scoring, EWMA, incident floors,
error density swap, scenario floor).

Root cause addressed: v3.95 A@1 DECREASES with budget (24.41→22.90) while
TraStrainer INCREASES (42.59→50.00). The AND gate triggers only at theta=beta
in v3.9, which at 2.5% budget allows a long "free" phase (only p_s gate) where
noisy traces fill up the budget. Budget-aware theta fixes this by being more
selective earlier. The score change ensures the final ranking respects budget:
at high budget, more weight on diversity and RCA leads to better MicroRank
contrast set.
"""
...
from streamv39_tuned_rca import (
    TracePoint, MetricVector, PreferenceVector, MetricStats,
    _DISABLE_OVERRIDE, _clamp, _median, _mad, _sigmoid, _mean, _std,
    _stable_positive_z, _trace_has_incident_service,
    _trace_relative_normalize, _rank_project,
    _compute_metric_signal_details, compute_sampling_probability,
    build_metric_inputs, _rca_quadrant_value,
)


def _composite_v71_budget_aware_alpha(
    ...
    # v7.1 change 1: budget-aware theta threshold and score weights
    if beta <= 0.0015:
        theta_strict = beta          # 100%: identical to v3.9
        w_sys, w_div, w_rca = 0.70, 0.10, 0.20
    elif beta <= 0.0125:
        theta_strict = 0.85 * beta   # 85%: tighten AND gate slightly
        w_sys, w_div, w_rca = 0.55, 0.15, 0.30
    else:
        theta_strict = 0.70 * beta   # 70%: much earlier AND gate
        w_sys, w_div, w_rca = 0.40, 0.25, 0.35
    ...
    for p in traces:
        ...
        lat_pressure = (0.7 * _norm(float(p.duration_ms), float(dur_lo), float(dur_hi)) +
        ...
        trace_metric_score, _, _signal_entropy = _compute_metric_signal_details(
            p, historical_preference_vector, historical_metric_stats
        )
        is_high_confidence_anomaly = trace_metric_score >= 3.0

        u = rng.random()
        # v7.1 change: theta_strict (budget-adaptive) instead of beta
        if theta > theta_strict:
            ...
            is_clean_normal = (
                (not p.has_error) and (incident_service <= 0.0) and
                (p_s < ps_or_floor) and (p_d <= 0.45)
            )
        ...
        # v7.1 change: budget-aware post-hoc score weights
        score_by_id[p.trace_id] = w_sys * p_s + w_div * p_d + w_rca * rca_value
    ...
    # Error density protection (same as v3.9)
    if kept_ids:
