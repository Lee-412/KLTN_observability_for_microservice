...
def _composite_v3_metrics_strictcap(
    ...
    enable_service_quota: bool = False,
    service_quota_k: int = 3,
    high_error_service_threshold: float = 0.2,
    max_budget_overflow_ratio: float = 0.10,
    service_quota_log: Optional[Dict[str, Any]] = None,
    ...
    if enable_service_quota and service_quota_k > 0 and kept_ids:
        quota_k = max(1, int(service_quota_k))
        err_threshold = _clamp(float(high_error_service_threshold), 0.0, 1.0)
        overflow_cap = max(0.0, float(max_budget_overflow_ratio))

        svc_total: Dict[str, int] = defaultdict(int)
        svc_error: Dict[str, int] = defaultdict(int)
        for trace_point in traces:
            for service in trace_point.services:
                svc_total[service] += 1
                if trace_point.has_error:
                    svc_error[service] += 1

        high_error_services = {
            service
            for service, total_cnt in svc_total.items()
            if total_cnt > 0 and (float(svc_error.get(service, 0)) / float(total_cnt)) > err_threshold
        }
        suspicious_services = set(incident_services) | high_error_services

        def _coverage(pool_ids: Set[str]) -> Dict[str, int]:
            coverage: Dict[str, int] = {svc: 0 for svc in sorted(suspicious_services)}
            for tid in pool_ids:
                trace_point = by_id.get(tid)
        ...
        coverage_before = _coverage(kept_ids)
        added_trace_count = 0

        # Enforce quota with anomaly-first, then v3.5 score descending.
        for service in sorted(suspicious_services):
            current_count = coverage_before.get(service, 0)
            if current_count >= quota_k:
                continue

            need = quota_k - current_count
            candidates = [
                tp.trace_id
                for tp in traces
                if (tp.trace_id not in kept_ids) and (service in tp.services)
            ]
            candidates.sort(
                key=lambda tid: (
        ...
        coverage_after = _coverage(kept_ids)
        quota_filled_services = [
            service
            for service in sorted(suspicious_services)
            if coverage_before.get(service, 0) < quota_k and coverage_after.get(service, 0) >= quota_k
        ]

        max_allowed_count = int(math.ceil(float(target_count) * (1.0 + overflow_cap)))
        if len(kept_ids) > max_allowed_count:
            non_suspicious_removable = [
                tid
                for tid in kept_ids
                if by_id.get(tid) and not (set(by_id[tid].services) & suspicious_services)
            ]
            non_suspicious_removable.sort(key=lambda tid: (score_by_id.get(tid, 0.0), tid))
            for tid in non_suspicious_removable:
                if len(kept_ids) <= max_allowed_count:
                    break
                kept_ids.discard(tid)

        budget_overflow_ratio = max(0.0, (len(kept_ids) - target_count) / float(max(1, target_count)))
        quota_log_row: Dict[str, Any] = {
            "coverage_before": coverage_before,
            "coverage_after": coverage_after,
            "quota_filled_services": quota_filled_services,
            "added_trace_count": added_trace_count,
            "budget_overflow_ratio": round(float(budget_overflow_ratio), 6),
            "suspicious_service_count": len(suspicious_services),
            "high_error_service_count": len(high_error_services),
            "quota_k": quota_k,
            "high_error_threshold": err_threshold,
            "max_budget_overflow_ratio": overflow_cap,
        }
        if service_quota_log is not None:
            service_quota_log.update(quota_log_row)
        if debug_trace_logs is not None:
            debug_trace_logs.append({"event": "service_quota", **quota_log_row})
