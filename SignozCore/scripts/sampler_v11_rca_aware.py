#!/usr/bin/env python3
"""V11 RCA-aware sampler.

This module extends V10 with:
1) RCA-aware relevance term in the objective,
2) less compressed metric evidence scale,
3) adaptive guard annealing after quota satisfaction,
4) bounded residual hard-constraint repair (<5% swaps),
5) coherence telemetry between pre/post repair selections.
"""
...
class V11RCAAwareSampler:
    """Set-aware sampler with RCA-aware objective and guard annealing."""

    def __init__(
        self,
        lambda_base: float = 0.40,
        mu_evidence: float = 0.25,
        lambda_rca: float = 0.25,
        eta_guard: float = 0.10,
        tau: float = 0.50,
        min_normal_ratio: float = 0.30,
        min_error_ratio: float = 0.25,
        redundancy_weight: float = 0.15,
        guard_floor_factor: float = 0.20,
        evidence_scale_mode: str = "rescale",
    ) -> None:
        self.lambda_base = max(0.0, float(lambda_base))
        self.mu_evidence = max(0.0, float(mu_evidence))
        self.lambda_rca = max(0.0, float(lambda_rca))
        self.eta_guard = max(0.0, float(eta_guard))
        self.tau = tau
        ...
        self.guard_floor_factor = _clamp(guard_floor_factor, 0.0, 1.0)
        mode = str(evidence_scale_mode or "rescale").strip().lower()
        self.evidence_scale_mode = mode if mode in {"rescale", "sigmoid"} else "rescale"
    ...
    def compute_evidence_score(
        ...
        if self.evidence_scale_mode == "sigmoid":
            ...
            # Keep dynamic range by avoiding a hard sigmoid bottleneck.
            metric_signal = math.log1p(max(0.0, metric_score))
            selectivity = 0.30 + 0.50 * coverage + 0.20 * confidence
            evidence = max(0.0, metric_signal * selectivity)
        return evidence, coverage, confidence

    def _build_service_rca_prior(
        self,
        ...
        service_error: Counter[str] = Counter()
        service_base_sum: Dict[str, float] = defaultdict(float)
        service_scenario_set: Dict[str, Set[int]] = defaultdict(set)

        for trace in traces:
            for service in trace.services:
                service_count[service] += 1
                if trace.has_error:
                    service_error[service] += 1
                service_base_sum[service] += float(base_by_id.get(trace.trace_id, 0.0))
                service_scenario_set[service] |= scenario_membership.get(trace.trace_id, set())
        ...
        for service, count in service_count.items():
            if count <= 0:
                prior[service] = 0.0
                continue
            error_rate = service_error[service] / float(count)
            base_mean = service_base_sum[service] / float(count)
            scenario_coverage = len(service_scenario_set[service]) / float(max(1, scenario_count))
            incident_bonus = 1.0 if service in incident_services else 0.0
            rarity = 1.0 / math.log1p(1.0 + count)
            prior[service] = _clamp(
                0.35 * error_rate
                + 0.30 * base_mean
                + 0.20 * scenario_coverage
                + 0.10 * incident_bonus
                + 0.05 * rarity,
                0.0,
                1.0,
    ...
    def compute_rca_relevance(
        self,
        trace: TracePoint,
        service_prior: Dict[str, float],
    ) -> float:
        if not trace.services:
            return 0.0
        priors = sorted((service_prior.get(service, 0.0) for service in trace.services), reverse=True)
        top = priors[:2]
        top_mean = _safe_mean(top)
        all_mean = _safe_mean(priors)
        concentration = 0.0 if all_mean <= 0.0 else _clamp(top_mean / all_mean, 0.0, 2.0) / 2.0
        return _clamp(0.75 * top_mean + 0.25 * concentration, 0.0, 1.0)
    ...
    def select_traces(
        ...
        service_prior = self._build_service_rca_prior(
            traces=traces,
        ...
        for trace in traces:
            rca_score = self.compute_rca_relevance(trace=trace, service_prior=service_prior)
            rca_by_id[trace.trace_id] = rca_score
            score_by_id[trace.trace_id] = (
                self.lambda_base * base_by_id[trace.trace_id]
                + self.mu_evidence * evidence_by_id[trace.trace_id]
                + self.lambda_rca * rca_score
            )
        ...
        for trace_id in ranked_ids:
            ...
            error_deficit = max(0, min_error_target - selected_error_count)
            normal_deficit = max(0, min_normal_target - selected_normal_count)
            deficit_total = error_deficit + normal_deficit
            deficit_denom = max(1, min_error_target + min_normal_target)
            quota_pressure = deficit_total / float(deficit_denom)
            ...
            if trace.services:
                ...
                overlap = 0.0
            adaptive_guard = self.eta_guard * (
                self.guard_floor_factor + (1.0 - self.guard_floor_factor) * quota_pressure
            )
            marginal = score_by_id[trace_id] + adaptive_guard * guard_score - self.redundancy_weight * overlap
...
def _composite_v11_rca_aware(
    ...
    selected_ids, score_by_id, diagnostics = sampler.select_traces(
        ...
        enable_residual_fix=enable_residual_fix,
    )
