"""
Stream v3.9 — tuned RCA-aware sampling.

v3.7 backbone + 3 targeted signal tunings (no architecture changes):

  1. Softer quadrant scoring:
     - Noise raised 0.10 -> 0.28 to stop over-punishing early error
       traces whose root-cause service hasn't accumulated enough
       history to enter the suspicious set (critical for train-ticket).
     - Gold 0.85+0.15r -> 0.80+0.10r, Silver 0.55->0.50, Mixed 0.35->0.40.

  2. Adaptive suspicious-service threshold:
     - Lowered from fixed 10 to 4 at tiny budgets (beta <= 0.15%),
       so root-cause services enter the suspicious set earlier.

  3. Stronger error-density swap:
     - min_err_ratio multiplier raised 1.2 -> 1.4  so more gold
       error traces survive post-hoc pruning.
"""
...
def _rca_quadrant_value(
    ...
    v3.9 tuning: Noise raised from 0.10 → 0.28 to avoid over-punishing
...
def _composite_v392_service_coverage(
    points: List[TracePoint],
    budget_pct: float,
    preference_vector: PreferenceVector,
    ...
    # v3.92: Service-coverage maximization post-hoc swap
    if kept_ids and len(kept_ids) >= 3:
        svc_count_kept: Dict[str, int] = defaultdict(int)
        for tid in kept_ids:
            tp = by_id.get(tid)
            if tp:
                for svc in tp.services:
                    svc_count_kept[svc] += 1
        all_svcs = set()
        for tp in traces:
            all_svcs.update(tp.services)
        missing_svcs = all_svcs - set(svc_count_kept.keys())
        max_swaps = max(1, len(kept_ids) // 5)
        swaps_done = 0
        for _ in range(max_swaps):
            ...
            best_cand = None
            best_new_svcs = 0
            for tid, tp in by_id.items():
                if tid in kept_ids:
                    continue
                new_svcs = len(set(tp.services) & missing_svcs)
                if new_svcs > best_new_svcs:
                    best_new_svcs = new_svcs
                    best_cand = tid
            if best_cand is None or best_new_svcs == 0:
                break
            drop_cands = []
            for tid in kept_ids:
                tp = by_id.get(tid)
                if tp and not tp.has_error:
                    min_svc_count = min(svc_count_kept.get(s, 0) for s in tp.services) if tp.services else 0
                    drop_cands.append((tid, min_svc_count, score_by_id.get(tid, 0.0)))
            if not drop_cands:
                break
            drop_cands.sort(key=lambda x: (-x[1], x[2]))
            drop_tid = drop_cands[0][0]
            drop_tp = by_id[drop_tid]
            kept_ids.discard(drop_tid)
            for svc in drop_tp.services:
                svc_count_kept[svc] = max(0, svc_count_kept.get(svc, 1) - 1)
                if svc_count_kept[svc] == 0:
                    missing_svcs.add(svc)
            kept_ids.add(best_cand)
            add_tp = by_id[best_cand]
            for svc in add_tp.services:
                svc_count_kept[svc] = svc_count_kept.get(svc, 0) + 1
                missing_svcs.discard(svc)
            swaps_done += 1
