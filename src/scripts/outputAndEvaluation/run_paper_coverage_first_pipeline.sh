...
SCENARIO_FILE="$(normalize_path "$SCENARIO_FILE")"
ENGINE_INPUT_DIR="$(normalize_path "$ENGINE_INPUT_DIR")"
...
echo "== STEP 1: ADAPTER =="
"$PYTHON_BIN" scripts/outputAndEvaluation/adapter_rca_engine_output.py \
...
"$PYTHON_BIN" scripts/outputAndEvaluation/evaluate_rca_ranking.py \
...
"$PYTHON_BIN" scripts/outputAndEvaluation/build_paper_table_from_summary.py \
