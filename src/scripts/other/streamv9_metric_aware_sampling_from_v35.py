...
# ---------------------------------------------------------------------------
# V9 metric-aware extension (keeps v3.5 baseline intact)
# ---------------------------------------------------------------------------
...
def _write_v9_telemetry(path: str, payload: Dict[str, Any]) -> None:
    out_path = Path(path)
...
# Backward-compatible alias for callers that still use the v8 writer name.
_write_v8_telemetry = _write_v9_telemetry
...
def _trace_metric_score(
    ...
    trace_score = _topk_projection(service_scores)
...
def _apply_contrast_health_guard(
    ...
    available_normal = len(normal_ranked)
    available_error = len(error_ranked)
    budget_relax = 1.0
    if budget_pct <= 0.15:
        budget_relax = 0.45
    elif budget_pct <= 1.0:
    ...
    eff_min_normal_ratio = min(float(min_normal_ratio) * budget_relax, available_normal / float(max(1, target_count)))
    eff_min_error_ratio = min(float(min_error_ratio) * budget_relax, available_error / float(max(1, target_count)))

    min_normal = max(0, min(target_count, int(round(target_count * eff_min_normal_ratio))))
    min_error = max(0, min(target_count, int(round(target_count * eff_min_error_ratio))))

    if (min_normal + min_error) > target_count:
        overflow = (min_normal + min_error) - target_count
        reduce_error = min(overflow, min_error)
        min_error -= reduce_error
...
def _enforce_scenario_floor_post_guard(
    selected_ids: Set[str],
    points: List[TracePoint],
    p_final_by_id: Dict[str, float],
    scenario_windows: List[Tuple[str, int, int]],
    ...
    if (
        target_count <= 0
        or not points
        or not scenario_windows
        or min_incident_traces_per_scenario <= 0
    ):
    ...
    if non_empty_scenarios == 0:
        return set(selected_ids), len(scenario_windows)
    ...
    if floor_each > 0:
        for idx, ids in enumerate(scenario_ids_by_idx):
            ...
            need = floor_each - len(present)
            candidates = [tid for tid in ids if tid not in final_ids]
            candidates.sort(key=lambda tid: (-_score(tid), tid))
            if not candidates:
                scenario_floor_fail_count += 1
                continue
            for tid in candidates[:need]:
    ...
    if len(final_ids) <= target_count:
        return final_ids, scenario_floor_fail_count
    ...
    removable = sorted(final_ids, key=lambda tid: (_score(tid), tid))
    for tid in removable:
        if len(final_ids) <= target_count:
            break
        if not _can_remove(tid):
    ...
    if len(final_ids) > target_count:
        forced = sorted(final_ids, key=lambda tid: (_score(tid), tid))
        for tid in forced:
            if len(final_ids) <= target_count:
                break
            final_ids.remove(tid)
...
def _composite_v9_metric_aware_sampling_from_v35(
    points: List[TracePoint],
    budget_pct: float,
    preference_vector: PreferenceVector,
    scenario_windows: List[Tuple[str, int, int]],
    ...
    metric_selectivity_values: List[float] = []
    ...
    def _metric_aware_sampling_probability(trace: TracePoint, context: Dict[str, Any]) -> float:
        ...
        metric_signal = _sigmoid(metric_score - float(tau))
        selectivity = _clamp(0.15 + 0.55 * coverage + 0.30 * confidence, 0.0, 1.0)
        delta_logit = float(lambda_eff) * float(logit_scale) * ((2.0 * metric_signal) - 1.0) * selectivity
        delta_logit = _clamp(delta_logit, -0.35, 0.75)
        p_final = _clamp(_sigmoid(_logit(p_trace) + delta_logit), 0.02, 0.98)
        ...
        metric_uplift_values.append(p_final - p_trace)
        mapping_coverage_values.append(coverage)
        mapping_confidence_values.append(confidence)
        metric_selectivity_values.append(selectivity)
    ...
    selected_ids, scenario_floor_fail_count = _enforce_scenario_floor_post_guard(
        selected_ids=selected_ids,
        points=points,
        p_final_by_id=p_final_by_id,
        scenario_windows=scenario_windows,
    ...
    if telemetry_output_path:
        ...
        telemetry = {
            ...
            "sampling": {
                ...
                "metric_selectivity_mean": _mean(metric_selectivity_values),
        ...
        _write_v9_telemetry(telemetry_output_path, telemetry)

    return selected_ids


def _composite_v8_metric_aware_sampling_from_v35(*args: Any, **kwargs: Any) -> Set[str]:
    # Compatibility shim for old callers.
    return _composite_v9_metric_aware_sampling_from_v35(*args, **kwargs)
