...
import src.scripts.MESC.streamv39_tuned_rca as sv39tunedcomp

import src.scripts.MESC.streamv391_contrast_rebalance as sv391comp
import src.scripts.MESC.streamv392_service_coverage as sv392comp
import src.scripts.MESC.streamv393_seed_ensemble as sv393comp
...
import src.scripts.MESC.streamv3941_metric_native_consensus as sv3941comp
import src.scripts.MESC.streamv3942_metric_native_signed_contrast as sv3942comp
import src.scripts.MESC.streamv395_contrast_ensemble as sv395comp
import src.scripts.other.streamv13_v36_swap_contrast as sv13comp
import src.scripts.other.streamv8_metric_aware_sampling_from_v35 as sv8comp
import src.scripts.other.sampler_v11_rca_aware as sv11comp
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.10,
        help="When selection-mode=stream-v13-v36-swap-contrast, outside-in replacement ratio over kept set",
    )
    ap.add_argument(
        "--v13-max-swap-ratio",
        type=float,
        default=0.15,
        help="When selection-mode=stream-v13-v36-swap-contrast, hard cap for replacement ratio",
    )
    ap.add_argument(
        "--v13-incident-window-sec",
        type=int,
        default=60,
        help="When selection-mode=stream-v13-v36-swap-contrast, local contrast window around incident anchor",
    )
    ap.add_argument(
        "--v37-rca-weight",
        type=float,
    ...
    ap.add_argument(
        "--v394s1-min-normal-ratio",
        type=float,
        default=0.35,
        help="When selection-mode=stream-v3.94-sprint1-metric-contrast-guard, minimum normal trace floor ratio",
    )
    ap.add_argument(
        "--v394s1-min-error-ratio",
        type=float,
        default=0.25,
        help="When selection-mode=stream-v3.94-sprint1-metric-contrast-guard, minimum error trace floor ratio",
    )
    ap.add_argument(
        "--v3941-min-consensus",
        type=int,
        default=2,
        help="When selection-mode=stream-v3.94.1-metric-native-consensus, minimum seed vote count required for consensus pool",
    )
    ap.add_argument(
        "--v3941-enable-swap-refine",
        action="store_true",
        help="When selection-mode=stream-v3.94.1-metric-native-consensus, enable optional v13-style bounded swap refine (v3.94.2 track)",
    )
    ap.add_argument(
        "--v3942-min-consensus",
        type=int,
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.25,
        help="When selection-mode=stream-v8-metric-aware-v35, metric fusion weight lambda_eff",
    )
    ap.add_argument(
        "--v8-tau",
        type=float,
        default=0.5,
        help="When selection-mode=stream-v8-metric-aware-v35, sigmoid center tau",
    )
    ap.add_argument(
        "--v8-min-normal-ratio",
        type=float,
        default=0.30,
        help="When selection-mode=stream-v8-metric-aware-v35, minimum normal ratio guarantee",
    )
    ap.add_argument(
        "--v8-min-error-ratio",
        type=float,
        default=0.25,
        help="When selection-mode=stream-v8-metric-aware-v35, minimum error ratio guarantee",
    )
    ap.add_argument(
    ...
    for spec in specs:
        ...
        for budget in budgets:
            ...
            if args.selection_mode == "ranked":
            ...
            elif args.selection_mode == "stream-v12-lite-bias-align":
                # V12lite: v3.5 signal core + incident-window bias alignment rerank.
                target_tps = 0.0
                preference_vector, metric_stats = sv3comp.build_metric_inputs(points)
                metric_dir = _metric_dir_from_trace_dir(spec.trace_dir)
                metrics_stream_by_sec = sv11comp.build_real_metrics_stream(str(metric_dir))

                # Budget-aware Stage-A weights:
                # - tight budget: precision-first (system-heavy)
                # - larger budget: more diversity/bias support
                if budget <= 0.1:
                    w_v35 = 0.75
                    w_div = 0.10
                    w_bias = 0.15
                    ...
                    t = (budget - 0.1) / (2.5 - 0.1)
                    w_v35 = 0.75 + t * (0.55 - 0.75)
                    w_div = 0.10 + t * (0.25 - 0.10)
                    w_bias = 0.15 + t * (0.20 - 0.15)

                min_floor = 1
                kept_ids = sv3comp._composite_v12_lite_bias_align(
                    points=points,
                    budget_pct=budget,
                    preference_vector=preference_vector,
                    metric_stats=metric_stats,
                ...
                "stream-v11-rca-aware",
                "stream-v11-no-rca-term",
                "stream-v11-fixed-guard",
                "stream-v11-adaptive-guard",
                "stream-v11-evidence-rescale",
            }:
                # V11 family: RCA-aware objective with clean ablations for guard policy and evidence scaling.
                target_tps = 0.0
                preference_vector, metric_stats = sv11comp.build_metric_inputs(points)
                metric_dir = _metric_dir_from_trace_dir(spec.trace_dir)
                metrics_stream_by_sec = sv11comp.build_real_metrics_stream(str(metric_dir))
                ...
                evidence_scale_mode = "rescale"
                ablation_variant = "v11-full"

                if args.selection_mode == "stream-v11-no-rca-term":
                    lambda_base = 0.45
                    mu_evidence = 0.35
                    lambda_rca = 0.0
                    eta_guard = 0.10
                    guard_floor_factor = 1.0
                    evidence_scale_mode = "sigmoid"
                    ablation_variant = "no_rca_term"
                elif args.selection_mode == "stream-v11-fixed-guard":
                    lambda_base = 0.40
                    mu_evidence = 0.25
                    lambda_rca = 0.25
                    eta_guard = 0.10
                    guard_floor_factor = 1.0
                    evidence_scale_mode = "sigmoid"
                    ablation_variant = "fixed_guard"
                elif args.selection_mode == "stream-v11-adaptive-guard":
                    lambda_base = 0.40
                    mu_evidence = 0.25
                    lambda_rca = 0.25
                    eta_guard = 0.10
                    guard_floor_factor = 0.20
                    evidence_scale_mode = "sigmoid"
                    ablation_variant = "adaptive_guard"
                elif args.selection_mode == "stream-v11-evidence-rescale":
                    lambda_base = 0.40
                    mu_evidence = 0.25
                    lambda_rca = 0.25
                    eta_guard = 0.10
                    guard_floor_factor = 0.20
                    evidence_scale_mode = "rescale"
                    ablation_variant = "evidence_rescale"

                min_floor = 1
                telemetry_path = root / "logs" / f"v11_telemetry_{ablation_variant}_{spec.name}_{budget_slug}.json"
                kept_ids = sv11comp._composite_v11_rca_aware(
                    points=points,
                    budget_pct=budget,
                    preference_vector=preference_vector,
                    metric_stats=metric_stats,
                    ...
                    eta_guard=eta_guard,
                    guard_floor_factor=guard_floor_factor,
                    evidence_scale_mode=evidence_scale_mode,
                    enable_residual_fix=True,
                    telemetry_output_path=str(telemetry_path),
                    dataset_name=spec.name,
                    budget_label=budget_slug,
            ...
            elif args.selection_mode == "stream-v8-metric-aware-v35":
                # v8: keep v3.5 baseline logic but inject real metric evidence into sampling probability.
                target_tps = 0.0
                preference_vector, metric_stats = sv8comp.build_metric_inputs(points)
                metric_dir = _metric_dir_from_trace_dir(spec.trace_dir)
                metrics_stream_by_sec = sv8comp.build_real_metrics_stream(str(metric_dir))

                min_floor = 1
                telemetry_path = root / "logs" / f"v8_telemetry_{spec.name}_{budget_slug}.json"
                kept_ids = sv8comp._composite_v8_metric_aware_sampling_from_v35(
                    points=points,
                    budget_pct=budget,
                    preference_vector=preference_vector,
                    metric_stats=metric_stats,
                    ...
                    lambda_eff=args.v8_lambda_eff,
                    tau=args.v8_tau,
                    min_normal_ratio=args.v8_min_normal_ratio,
                    min_error_ratio=args.v8_min_error_ratio,
                    telemetry_output_path=str(telemetry_path),
                    dataset_name=spec.name,
                    budget_label=budget_slug,
                )
            ...
            elif args.selection_mode == "stream-v13-v36-swap-contrast":
                # V13: preserve full v3.6 stable core, then apply small outside-in contrast-aware swaps.
                target_tps = 0.0
                preference_vector, metric_stats = sv13comp.build_metric_inputs(points)
                v13_stats: dict[str, Any] = {}

                min_floor = 1
                kept_ids = sv13comp.stream_sampler_v13(
                    points=points,
                    budget_pct=budget,
                    preference_vector=preference_vector,
                    metric_stats=metric_stats,
                    ...
                    min_incident_traces_per_scenario=min_floor,
                    online_soft_cap=(args.budget_mode != "strict"),
                    swap_ratio=args.v13_swap_ratio,
                    max_swap_ratio=args.v13_max_swap_ratio,
                    incident_window_sec=args.v13_incident_window_sec,
                    debug_stats=v13_stats,
                )
                achieved_keep_pct = (len(kept_ids) / len(points) * 100.0) if points else 0.0
                pre_cap_keep_pct = achieved_keep_pct
                sim_metrics = {
                    ...
                    "threshold_volatility_pct": None,
                    "max_step_change_pct": None,
                    "v13_swap_quota": v13_stats.get("swap_quota"),
                    "v13_swap_count": v13_stats.get("swap_count"),
                    "v13_min_error_count": v13_stats.get("min_error_count"),
                    "v13_min_normal_count": v13_stats.get("min_normal_count"),
                    "v13_final_error_count": v13_stats.get("final_error_count"),
                    "v13_final_normal_count": v13_stats.get("final_normal_count"),
                    "v13_global_error_ratio": v13_stats.get("global_error_ratio"),
                }
                print(
                    "[v13-v36-swap-contrast] "
                    f"swaps={v13_stats.get('swap_count', 0)}/{v13_stats.get('swap_quota', 0)} "
                    f"err={v13_stats.get('final_error_count', 0)} "
                    f"normal={v13_stats.get('final_normal_count', 0)}"
                )
                ...
                pre_cap_keep_pct = achieved_keep_pct
                sim_metrics = {"incident_capture_latency_ms": None, "threshold_volatility_pct": None, "max_step_change_pct": None}
            elif args.selection_mode == "stream-v3.94.1-metric-native-consensus":
                target_tps = 0.0
                preference_vector, metric_stats = sv3941comp.build_metric_inputs(points)
                metric_dir = _metric_dir_from_trace_dir(spec.trace_dir)
                metrics_stream_by_sec = sv39tunedcomp.build_real_metrics_stream(str(metric_dir))
                min_floor = 1
                kept_ids = sv3941comp._composite_v3941_metric_native_consensus(
                    points=points,
                    budget_pct=budget,
                    preference_vector=preference_vector,
                    metric_stats=metric_stats,
                    ...
                    metrics_stream_by_sec=metrics_stream_by_sec,
                    metric_lookback_sec=args.v8_metric_lookback_sec,
                    min_consensus=args.v3941_min_consensus,
                    enable_swap_refine=args.v3941_enable_swap_refine,
                )
                achieved_keep_pct = (len(kept_ids) / len(points) * 100.0) if points else 0.0
                pre_cap_keep_pct = achieved_keep_pct
                sim_metrics = {"incident_capture_latency_ms": None, "threshold_volatility_pct": None, "max_step_change_pct": None}
            elif args.selection_mode == "stream-v3.94.2-metric-native-signed-contrast":
                target_tps = 0.0
                ...
                pre_cap_keep_pct = achieved_keep_pct
                sim_metrics = {"incident_capture_latency_ms": None, "threshold_volatility_pct": None, "max_step_change_pct": None}
            elif args.selection_mode == "stream-v3.94-sprint1-metric-contrast-guard":
                target_tps = 0.0
                preference_vector, metric_stats = sv394s1comp.build_metric_inputs(points)
                metric_dir = _metric_dir_from_trace_dir(spec.trace_dir)
                metrics_stream_by_sec = sv39tunedcomp.build_real_metrics_stream(str(metric_dir))
                min_floor = 1
                kept_ids = sv394s1comp._composite_v394_sprint1_metric_contrast_guard(
                    points=points,
                    budget_pct=budget,
                    preference_vector=preference_vector,
                    metric_stats=metric_stats,
                    ...
                    metrics_stream_by_sec=metrics_stream_by_sec,
                    metric_lookback_sec=20,
                    min_normal_ratio=args.v394s1_min_normal_ratio,
                    min_error_ratio=args.v394s1_min_error_ratio,
                    emit_debug=True,
                )
                achieved_keep_pct = (len(kept_ids) / len(points) * 100.0) if points else 0.0
                pre_cap_keep_pct = achieved_keep_pct
            ...
            elif args.selection_mode == "stream-v7.1-budget-aware-alpha":
                # v7.1: v3.9 backbone + budget-aware AND gate threshold + budget-aware score weights.
                target_tps = 0.0
                preference_vector, metric_stats = sv71comp.build_metric_inputs(points)
                min_floor = 1
                kept_ids = sv71comp._composite_v71_budget_aware_alpha(
                    points=points,
                    budget_pct=budget,
                    preference_vector=preference_vector,
                    scenario_windows=scenario_windows,
            ...
            elif args.selection_mode == "stream-v7.2-two-stage-gating":
                # v7.2: deterministic two-stage gating (full population scan + diversity filter).
                target_tps = 0.0
                preference_vector, metric_stats = sv72comp.build_metric_inputs(points)
                min_floor = 1
                kept_ids = sv72comp._composite_v72_two_stage_gating(
                    points=points,
                    budget_pct=budget,
                    preference_vector=preference_vector,
                    scenario_windows=scenario_windows,
